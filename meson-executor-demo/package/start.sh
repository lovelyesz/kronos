...
CONTAINER_MAIN_JAR=meson-executor-demo-1.0.0.jar
echo $DEPLOY_DIR
...
echo $JAVA_HOME/bin/java ${JVM_PARAM} -jar ${DEPLOY_DIR}/demo-meson/lib/${CONTAINER_MAIN_JAR}

$JAVA_HOME/bin/java ${JVM_PARAM} -jar ${DEPLOY_DIR}/demo-meson/lib/${CONTAINER_MAIN_JAR} --spring.profiles.active=prod
